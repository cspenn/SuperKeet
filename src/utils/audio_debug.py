...
class MemoryMonitor:
    ...
    def force_garbage_collection(self) -> int:
        ...
        self.logger.debug(f"🗑️ Forced GC: {collected} collections, {before_count - after_count} objects freed")
        return collected
    ...
    def periodic_cleanup(self) -> None:
        ...
        should_gc = (
            memory_info.get("rss_mb", 0) > 500 or  # High memory usage
            self.check_memory_pressure() or        # System memory pressure
            current_time - self._last_gc_time > 300  # Haven't GC'd in 5 minutes
        )
        
        if should_gc:
            self.logger.info(f"💾 Memory cleanup triggered - RSS: {memory_info.get('rss_mb', 0):.1f}MB")
