...
class SuperKeetApp:
    ...
    def _on_hotkey_pressed(self) -> None:
        """Handle hotkey press event with robust audio error handling."""
        if self.state != AppState.IDLE:
        ...
        # Try to start recording - now returns boolean instead of raising exception
        recording_started = self.audio_recorder.start()
        ...
        if not recording_started:
            ...
            try:
                if self.tray_icon.supportsMessages():
                    self.tray_icon.showMessage(
                        "SuperKeet Audio Error",
                        "Cannot access microphone. Please check permissions and try "
                        "again.",
